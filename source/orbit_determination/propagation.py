...
from orbit_determination.alpha5 import to_alpha5
...
def _dpper(satrec, inclo, init, ep, inclp, nodep, argpp, mp, opsmode):
     ...
     #  --------------- 시간 변화 주기 계산 -----------
     zm    = zmos + zns * t;
     # initial call time 이 0으로 설정되어 있는지 확인
...
*    vallado, crawford, hujsak, kelso  2006
  ----------------------------------------------------------------------------*/
"""
# Deep space 섭동 모델의 전처리 알고리즘으로 이후 사용하는 ddper, dsinit 에서 공통적으로 쓰이는 변수 계산
def _dscom(
     ...
     # 첫번째 루프에서 태양 계수 저장, 두번째 루프에서는 달 계수 계산 및 저장장
     for lsflg in 1, 2: 
...
*    vallado, crawford, hujsak, kelso  2006
  ----------------------------------------------------------------------------*/
"""
# deepspace 공명과 섭동을 고려한 초기화 알고리즘
def _dsinit(
       xke,
...
*    vallado, crawford, hujsak, kelso  2006
  ----------------------------------------------------------------------------*/
"""
# deep space 공명 효과 및 외력의 영향을 수치적으로 보정하는 수치적분
def _dspace(
       irez,
...
*    vallado, crawford, hujsak, kelso  2006
  ----------------------------------------------------------------------------*/
"""
# SGP4의 초기화 알고리즘으로 위성 궤도 요소와 지구 상수들을 기반으로 한 보조 변수들을 계산
def _initl(
